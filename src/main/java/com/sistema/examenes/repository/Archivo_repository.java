...
public interface Archivo_repository extends JpaRepository<Archivo_s, Long> {
    ...
    @Query("SELECT ar FROM Archivo_s ar " +
            "JOIN ar.actividad asig " +
            "JOIN asig.usuario u " +
            "WHERE u.username = :username AND ar.visible = true")
    public List<Archivo_s> listararchivouser(@Param("username") String username);
    @Query("SELECT ar FROM Archivo_s ar WHERE ar.visible = true AND ar.actividad.id_asignacion_evidencia = :idActividad")
    public List<Archivo_s> listararchivoActividad(@Param("idActividad") Long idActividad);
