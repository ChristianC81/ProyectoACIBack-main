...
public interface Modelo_repository extends JpaRepository<Modelo, Long> {
    ...
    @Query(value = "SELECT cri.nombre AS criterionomj, " +
            ...
            "criterio cri " +
            "JOIN " +
            "subcriterio sub ON cri.id_criterio = sub.id_criterio AND sub.visible = true " +
            "LEFT JOIN " +
            "indicador i ON sub.id_subcriterio = i.subcriterio_id_subcriterio AND i.visible = true " +
            "LEFT JOIN " +
            "asignacion_indicador ai ON i.id_indicador = ai.indicador_id_indicador " +
            "LEFT JOIN " +
            "evidencia ev ON i.id_indicador = ev.indicador_id_indicador AND ev.visible = true " +
            "LEFT JOIN " +
            "asignacion_evidencia ac ON ev.id_evidencia = ac.evidencia_id_evidencia AND ac.visible = true " +
            "LEFT JOIN " +
            "archivo arc ON ac.id_asignacion_evidencia = arc.id_asignacion_evidencia AND arc.visible = true " +
            "WHERE " +
            "cri.id_criterio = :id_criterio " +
            "AND ai.modelo_id_modelo = :id_modelo " +
            "ORDER BY " +
            "cri.id_criterio, " +
            "sub.id_subcriterio, " +
            "i.id_indicador", nativeQuery = true)
